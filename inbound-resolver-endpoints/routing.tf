# Create return routes for each subnet and route combination
resource "aws_route" "return_routes" {
  for_each = {
    for pair in setproduct(keys(var.resolver_subnets), keys(var.return_routes)) :
    "${pair[0]}-${pair[1]}" => {
      subnet_key   = pair[0]
      subnet_id    = var.resolver_subnets[pair[0]].subnet_id
      route_name   = pair[1]
      route_config = var.return_routes[pair[1]]
    }
  }
